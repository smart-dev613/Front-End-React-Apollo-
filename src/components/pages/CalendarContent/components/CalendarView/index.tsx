...
const CalendarView = (props: any) => {
  ...
  const prevWeek = (): void => {
    ...
    if (calendarAPI) {
      ...
      setTitle(prevDate.format('MMMM YYYY'));
    }
  };
  ...
  const nextWeek = (): void => {
    ...
    if (calendarAPI) {
      ...
      setTitle(nextDate.format('MMMM YYYY'));
    }
  };
    ...
    <>
      ...
      <FullCalendar
        ...
        datesSet={(value: any) => {
          ...
          setTitle(currentDate.format('MMMM YYYY'));
        }}
        events={events}
        ...
        eventContent={({ event }: any) => {
          return (
            <>
              <b>{event.extendedProps.invitationStatus === 'AWAITING' ? 'Pending' : event.title ?? ''}</b>
            </>
          );
        }}
      />
